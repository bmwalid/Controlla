...
spark = SparkSession.builder.appName("dpp_hierarchie_modele").enableHiveSupport().getOrCreate()
...
tableFinale.repartition(80).write.option("compression", "snappy").mode("overwrite").format("parquet").saveAsTable(
    "kylin_usb_mqb.dpp_hierarchie_modele")
tableFinale.select([col(c).cast("string") for c in tableFinale.columns]).coalesce(1).write.option("sep", ";").mode(
    "overwrite").save("s3://preprod-decathlon-kylin/MyQualityBoard/lookup_tables/003-dpp_hierarchie_modele_csv/", format='csv',
                      header=True)
